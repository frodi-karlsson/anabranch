...
Deno.test({
  ...
  async fn() {
    ...
    assertInstanceOf(first.error, FSErrors.NotFound);
...
Deno.test({
  ...
  async fn() {
    ...
    const err = (result as { error: FSError }).error;
    assertInstanceOf(err, FSError);
    assertInstanceOf(err, FSErrors.NotFound);
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const err = (result as { error: FSError }).error;
      assertInstanceOf(err, FSError);
      assertInstanceOf(err, FSErrors.InvalidData);
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      assertEquals(result.type, "success");
      assertEquals(
        (result as { value: Uint8Array }).value,
        data,
      );
    } finally {
      await Deno.remove(dir, { recursive: true });
